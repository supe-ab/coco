...
public class ItemServiceImplTest {
    ...
    @Test
    void testCreateItemFallback() {
        // Simulate exception during the service call
        RuntimeException exception = new RuntimeException("Simulated failure");
        ItemVO fallbackItem = itemService.createItemFallback(itemVO, exception);

        assertNotNull(fallbackItem);
        assertEquals("Fallback Item", fallbackItem.getName());
        assertEquals(0, fallbackItem.getQuantity());
    }
}
